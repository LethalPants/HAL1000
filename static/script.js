...
document.getElementById("image").addEventListener("change", ev => {
    ...
    imageBox.innerHTML = `<img src=${URL.createObjectURL(image)} alt="picture">`;
    ...
    fetch("/predict", {
        method: 'POST',
        body: formData
    }).then(res => res.json()).then(data => makeSentence(data)).catch(err => {
...
function makeSentence(data) {
    if (data.success === true) {
        ...
        errorContainer
            .innerHTML = `<div class="error"> Something went wrong. Please reload the page and try again. </div>`
    }
}